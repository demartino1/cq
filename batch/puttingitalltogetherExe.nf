...
params.out = "$launchDir/alltogether"
...
process downloadSAM {
  ...
  input:
    val inurl
  output:
    path "sequences.sam.1"
	//downloading the file with the variable params.url
	"""
	wget $inurl -O sequences.sam.1
	"""
}
...
process cleanSAM {
  ...
  input:
    path infile
  output:
    path "cleaned_sequences.sam.1"
  """
  cat $infile | grep "Sequence" > cleaned_sequences.sam.1
  """
}
